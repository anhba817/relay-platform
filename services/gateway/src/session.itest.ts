...
describe("the socket's delivery, with a fan-out attached", () => {
  ...
  /** T009 — RED ON PURPOSE, and the red is not the one the task
   * predicted.
   *
  ...
  /** T009's RED HALF — and it is red for one phase, not four.
   *
   ...
   * `packages/e2e/src/tuan.itest.ts`, the suite that guards the retry-and-disable chapter's
   * duplicate delivery, which is the defect the twenty-run battery exists for.
   * Measured: the lane read 40 files and 693 tests against the membership-revocation chapter's
