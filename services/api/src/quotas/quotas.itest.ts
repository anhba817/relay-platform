...
describe("the month's usage", () => {
  ...
  it("counts every send toward a user, because no send can be unattributed", async () => {
    // THIS TEST ASSERTED THE OPPOSITE AND THE PLATFORM CHANGED UNDER IT. It read
    // "counts an unattributed send toward messages and toward no user", on the
    // grounds that a key-authenticated REST send carries no user — true of a
    // `sendMessage` whose `userId` was optional. The sender chapter made it
    // REQUIRED, so the call no longer compiles and the state it described cannot be
    // reached from any write path. `activeUsers` was 0 there; it is 1 here, and the
    // difference is a requirement rather than a behaviour change in this chapter.
    //
    // Kept rather than deleted, because the property it now states is the one the
    // roll-up depends on: the distinct-user count IS the count of senders, with no
    // arm for a message that has none.
    const { environmentId, repo, channelId } = await seed();
    await repo.sendMessage(channelId, { userId: await newUser(repo), text: "hi" });
...
describe("running out", () => {
  ...
  it("refuses the send and serves the history read, in the same second", async () => {
    ...
    // because the requirement is about the pair.
...
describe("nobody is surprised", () => {
  ...
  it("sends exactly three emails per quota per period, and Mailpit has them", async () => {
    ...
    // fourth message clears 80% and 100% in one step, because four fifths of four is
    // not a whole message
  ...
  it("cannot fail a send when the mail server is gone", async () => {
    // WRITING A ROW IS NOT SENDING ONE, and this is the requirement that says so out
    // loud. The mail-transport chapter met the same hazard from the other side,
...
// codebase that no test enters — the outbox chapter's, the webhook dispatcher
// chapter's and the mail-transport chapter's all report the same hole, and
// `notification-relay.ts` sits at 58.06% statements because of it. Every suite
// drives `drainOnce()` directly, which is the right way to
